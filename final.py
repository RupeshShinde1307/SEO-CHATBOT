...
def load_self_hosted_llm():
    try:
        ...
        st.error(f"⚠️ Error loading self-hosted LLM: {str(e)}")
        return None
...
**Answer:** Provide a concise, bulleted response using only the information from the context. Avoid repetition and ensure clarity.
""",
        input_variables=["context", "question"]
    )
...
def clean_and_format_response(response):
    ...
    for sent in sentences:
        if sent.strip() and sent not in seen:
            seen.add(sent)
...
def generate_response(prompt, vectorstore, llm):
    ...
    try:
        ...
        if not result.strip() or "I'm sorry" in result or "I can only provide answers" in result:
            return "❌ Sorry, but I can only provide answers based on the available documents."
